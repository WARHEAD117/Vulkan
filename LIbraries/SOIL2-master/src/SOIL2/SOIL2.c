...
unsigned char*
SOIL_load_image_full
(
	const char *filename,
	int *width, int *height, int *channels, int *faces, int *mipmaps, int *bit_depth,
	int force_channels
)
{
	unsigned char *result = stbi_load_full(filename,
		width, height, channels, faces, mipmaps, bit_depth, force_channels);
	if (result == NULL)
	{
		result_string_pointer = stbi_failure_reason();
	}
